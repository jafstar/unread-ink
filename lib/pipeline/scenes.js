...
// Real bug found live, reading actual scene illustrations: when a scene
// DID include the character, the imageDescription often only described
// the surroundings ("hovering tablets... pulsing with light") without
// saying what the character was DOING - so the reference-conditioned
// image generator (Flux) had nothing to work from except the reference
// photo itself, and defaulted to just planting that exact standing,
// front-facing portrait pose into the new background. Looked like she
// was posing for a book cover instead of being IN the scene. Now
// explicit: whenever hasCharacter is true, the description must specify
// their pose/action/orientation, never just their presence.
const SYSTEM = 'You split a chapter of a novel into its real narrative scenes - a new scene starts wherever the setting changes, real time passes, or the focus of the action shifts, not at a fixed word count. Reproduce the chapter\'s own prose verbatim, just split at those scene boundaries - do not rewrite, summarize, or change any wording. For each scene, also write a plain, concrete one-to-two sentence illustration description: what the scene physically looks like. A scene does NOT need a character in it - some of the best illustrations here are pure setting and atmosphere (the ship itself, the weather, the dock, a room, an object described in the text) with nobody in frame at all. Only include a character if they are genuinely the visual focus of that specific passage - and when you do, the description MUST specify their concrete pose, action, and body orientation as described or implied by the text (reaching toward something, crouched low, in profile studying an object, mid-stride, turned away from camera) - NEVER just their presence or a generic standing pose. The illustrator needs to know what the character is physically doing in this specific moment, not just that they exist in the frame. No prose style, no metaphor, just the visual facts an illustrator would need. Output ONLY valid JSON, no markdown fences, no commentary, matching this exact shape: {"scenes": [{"text": string, "imageDescription": string, "hasCharacter": boolean}]}'
