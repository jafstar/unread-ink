...
export async function relayWriteChapter1({ winningPlot, bigAnswer, writerKeys, keys }) {
  ...
  for (let i = 0; i < writerKeys.length; i++) {
    ...
    const prompt = isFirst
      ? `Book premise: ${bigAnswer}\n\nWinning plot:\n${winningPlot}\n\nWrite Chapter 1 now - the opening of this book.`
      : `Book premise: ${bigAnswer}\n\nWinning plot:\n${winningPlot}\n\nHere is the chapter so far:\n\n${draft}\n\nContinue and refine it per your instructions.`
...
export async function relayWriteChapterChecked({ bigAnswer, winningPlot, chapterNum, chapterTitle, chapterSummary, priorChapters, keys }) {
  const bridgeInstruction = priorChapters
    ...
    : ''
  const context = `Book premise: ${bigAnswer}\n\nOverall plot:\n${winningPlot}\n\n${priorChapters ? `What has happened so far:\n\n${priorChapters}\n\n` : ''}Chapter ${chapterNum}: "${chapterTitle}", which covers: ${chapterSummary}${bridgeInstruction}`
  ...
  draft = await checkContinuity({ draft, winningPlot, priorChapters, chapterTitle, chapterSummary, apiKey: keys.claude })
