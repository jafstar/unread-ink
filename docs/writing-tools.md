...
### 7. Pass 2 — Whole-Manuscript Construction Audit (diagnostic only)
**File:** `lib/pipeline/passTwo.js`, `scripts/storyglue-pass2.mjs`

Real gap this exists to close: an outside review caught the book repeating a "Not X — Y" reveal construction 7 times across the book, invisible to every per-chapter tool above - continuity/lexical checks only ever compare a chapter against what came before it, nothing read the whole finished book as one document and asked "does this repeat the same sentence CONSTRUCTION, not just the same word." Pass 2 runs once a book is fully drafted: a single Claude call reads the entire manuscript (or a chapter range, for scoping a run to match what a specific external review covered) and reports repeated grammatical shapes and rhythms with real quoted examples, chapter locations, and approximate counts - explicitly instructed to never give a neutral "prose reads well" verdict, name real patterns concretely or say plainly that none were found. First real run (chapters 1-3, matching an outside review's exact scope) independently confirmed both patterns that review caught, plus three more it didn't mention (em-dash mid-sentence corrections, a "bodily sensation + explicit metaphor denial" tic, a triadic-rhythm habit).

**Deliberately diagnostic only** - it reports findings, it does not rewrite anything. A "Pass 3" (the actual rewrite mechanic, plus the ability to redirect the story's direction, not just polish its prose) is scoped as later work, not built here.

---

## Planned, not yet built

- **Pass 3 (rewrite mechanic + creative redirection)** — Pass 2's findings currently just sit in a report for a human to act on. Pass 3 would close that loop: actually rewrite the flagged passages, and separately, let a human steer the finished draft in a genuinely different direction, not just polish what exists. Deliberately not built alongside Pass 2 - diagnostic and rewrite are being kept as two different tools on purpose, not folded together.
